frappe.pages['em-order'].on_page_load = function(wrapper) {
	new EmOrder(wrapper)
}
...
EmOrder = Class.extend(
	{
		...
		// alert(currdate)
		frappe.call({
        method: "prime.api.get_orders.get_em_orders", //dotted path to server method
		...
		//  let fields = frappe.get_meta("Sales Order").fields
			...
			{title:"Doctor Name", field:"ref_practitioner"},
			{title:"Total", field:"total"},
			{title:"Time", field:"modified",formatter:formatter},
		...
		tbldata.forEach(row => {
			...
			// console.log("this Name :"+row.name+" Status: "+row.status+" per bill "+row.per_billed)
			let btnhml = ''
...
cash_sales = function(source_name){
	frappe.model.open_mapped_doc({
		method: "prime.api.make_invoice.make_emergency_invoice",
		source_name: source_name
	})
