...
def get_stats():
    ...
    revenue_this_month = scalar(
        "SELECT COALESCE(SUM(grand_total),0) FROM `tabSales Invoice` WHERE posting_date>=%s AND docstatus=1", first_this)
    collected_this_month = scalar(
        "SELECT COALESCE(SUM(grand_total-outstanding_amount),0) FROM `tabSales Invoice` WHERE posting_date>=%s AND docstatus=1", first_this)
    revenue_last_month = scalar(
        "SELECT COALESCE(SUM(grand_total),0) FROM `tabSales Invoice` WHERE posting_date BETWEEN %s AND %s AND docstatus=1",
        first_last, last_last)
    total_outstanding = scalar(
        "SELECT COALESCE(SUM(outstanding_amount),0) FROM `tabSales Invoice` WHERE outstanding_amount>0 AND docstatus=1")
    total_invoices = scalar(
        "SELECT COUNT(*) FROM `tabSales Invoice` WHERE posting_date>=%s AND docstatus=1", first_this)
    paid_invoices = scalar(
        "SELECT COUNT(*) FROM `tabSales Invoice` WHERE posting_date>=%s AND docstatus=1 AND outstanding_amount=0", first_this)
    ...
    # ── Revenue by insurance vs cash ──────────────────────────────────────────
    insurance_split = q("""
        SELECT CASE WHEN is_insurance=1 THEN 'Insurance' ELSE 'Direct Pay' END AS label,
               COALESCE(SUM(grand_total),0) AS value
        FROM `tabSales Invoice`
        WHERE posting_date>=%s AND docstatus=1
        GROUP BY is_insurance
    """, first_this)

    # ── Payment mode breakdown (from Que this month) ───────────────────────────
    mode_split = q("""
        SELECT COALESCE(mode_of_payment,'Unknown') AS label,
               COUNT(*) AS value
        FROM `tabQue`
    ...
    top_services = q("""
        SELECT item_name AS service,
               COALESCE(SUM(amount),0) AS revenue,
               COUNT(*) AS qty
        FROM `tabSales Invoice Item`
        WHERE parent IN (
            SELECT name FROM `tabSales Invoice` WHERE posting_date>=%s AND docstatus=1
        )
