...
frappe.pages['approved'].on_page_load = function(wrapper) {
	new ApprovedTest(wrapper)
}

ApprovedTest = Class.extend(
	{
		init:function(wrapper){
			...
			let myf = this
			this.colms = []
			frappe.realtime.on('lab_result_update', (data) => {
				// alert("in realtime")
				myf.setupdata_table()
					})
			this.grouping_cols()
		...
		let tbldata = []
		frappe.call({
        method: "prime.api.labtsamples.completed", //dotted path to server method
		...
		//  let fields = frappe.get_meta("Sales Order").fields
			...
			{title:"Sample", field:"sample" ,  headerFilter:"input",},
			{title:"Diagnosis", field:"diagnosis" ,  headerFilter:"input", formatter:"html"},
		...
		// 	// row['status'] = "<span style='color:green'>Completed</span>"
		// 	// row['action'] = btnhml
		// 	if(row['status'] == null){
		// 		row['status'] = ""
		// 	}
		// 	row['status'] = `<span style='color:blue'>${row['status']}</span>`
			
		// 	new_data.push(row)
		// })
		// console.log(columns)
this.table = new Tabulator("#approve_", {
			// layout:"fitDataFill",
		 ...
		 this.table.on("rowClick", function(e ,rows){
			...
			//  row.toggle_actions_menu_button(row.table.getSelectedData().length > 0);
			 frappe.set_route("Form" , "Lab Result" , rows._row.data.name)
...
<div id="approve_" style = "min-width : 100%"></div>
