...
def get_stats():
    ...
    patients_today     = scalar("SELECT COUNT(*) FROM `tabPatient Appointment` WHERE appointment_date=%s AND status!='Cancelled'", (td,))
    patients_month     = scalar("SELECT COUNT(*) FROM `tabPatient Appointment` WHERE appointment_date>=%s AND status!='Cancelled'", (first_day,))
    ...
    revenue_month      = scalar("SELECT COALESCE(SUM(grand_total),0) FROM `tabSales Invoice` WHERE posting_date>=%s AND docstatus=1", (first_day,))
    pending_invoices   = scalar("SELECT COUNT(*) FROM `tabSales Invoice` WHERE outstanding_amount>0 AND docstatus=1")
    outstanding_amount = scalar("SELECT COALESCE(SUM(outstanding_amount),0) FROM `tabSales Invoice` WHERE outstanding_amount>0 AND docstatus=1")
    queue_active       = scalar("SELECT COUNT(*) FROM `tabQue` WHERE date=%s AND status NOT IN ('Completed','Cancelled')", (td,))
    new_patients       = scalar("SELECT COUNT(*) FROM `tabPatient` WHERE DATE(creation)=%s", (td,))

    # Monthly revenue — no % chars, no params needed
    ...
    top_doctors = q(
        ...
        " FROM `tabPatient Appointment`"
        " WHERE appointment_date>=%s AND status!='Cancelled'"
        " GROUP BY practitioner ORDER BY patients DESC LIMIT 6",
        (first_day,)
    )

    queue_status = q(
        "SELECT COALESCE(status,'Unknown') AS label, COUNT(*) AS value"
        " FROM `tabQue` WHERE date=%s GROUP BY status",
        (td,)
    )
