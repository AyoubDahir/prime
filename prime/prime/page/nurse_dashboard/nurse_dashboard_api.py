...
def get_stats():
    ...
    queue_waiting   = scalar("SELECT COUNT(*) FROM `tabQue` WHERE date=%s AND status='Waiting'", (td,))
    queue_called    = scalar("SELECT COUNT(*) FROM `tabQue` WHERE date=%s AND status='Called'", (td,))
    queue_completed = scalar("SELECT COUNT(*) FROM `tabQue` WHERE date=%s AND status='Completed'", (td,))
    queue_total     = scalar("SELECT COUNT(*) FROM `tabQue` WHERE date=%s", (td,))
    vitals_today    = scalar("SELECT COUNT(*) FROM `tabVital Signs` WHERE DATE(creation)=%s AND docstatus!=2", (td,))
    lab_samples     = scalar("SELECT COUNT(*) FROM `tabSample Collection` WHERE DATE(creation)=%s", (td,))
    appt_today      = scalar("SELECT COUNT(*) FROM `tabPatient Appointment` WHERE appointment_date=%s AND status!='Cancelled'", (td,))
    appt_seen       = scalar("SELECT COUNT(*) FROM `tabPatient Appointment` WHERE appointment_date=%s AND status='Closed'", (td,))
    ...
    return {
        "kpis": {
            ...
            "lab_samples_today": int(lab_samples),
            "appointments_today": int(appt_today),
            "appointments_seen": int(appt_seen),
        },
