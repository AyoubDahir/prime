...
frappe.ui.form.on('Patient History', {
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__("Wrist band"), function(){
    const printName = "Wrist band";
    ...
    const params = {
        ...
        _lang: (frappe.boot && frappe.boot.lang) ? frappe.boot.lang : 'en'
    };

    const qs = Object.keys(params).map(k => `${encodeURIComponent(k)}=${encodeURIComponent(params[k])}`).join('&');
    const url = frappe.urllib.get_full_url(`/printview?${qs}`);
    const w = window.open(url, '_blank');
    if (!w) {
        frappe.msgprint(__('Popup blocked. Please allow popups for this site.'));
        return;
    }
    ...
    setTimeout(function(){
		...
		if (frappe.user_roles.includes('Doctor')) {
			...
			var btn = frm.add_custom_button('Discharge', () => {
				...
				//                     () => {
				//             frappe.call({
				//             method: "prime.api.discharge.patient_clearance", //dotted path to server method
				//             args: {
		...
		.then( ip => {
			...
			let dialog = new frappe.ui.Dialog({
				...
				primary_action : function() {
					...
					frappe.call({
						// doc: frm.doc,
						method: 'prime.api.transfer_ip.transfer_ip',
						args:{
							'self' : ip.name,
			...
			let d = new frappe.ui.Dialog({
				...
				primary_action(values) {
					...
					// console.log()
						...
						frappe.call({
							method: 'prime.prime.doctype.patient_history.patient_history.refer',
							args: {
