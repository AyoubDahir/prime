...
frappe.ui.form.on('GYN', {
	...
	refresh: function(frm) {
		...
		// 	frappe.new_doc("Patient Encounter",{"patient": frm.doc.patient,"practitioner" : frm.doc.practitioner,}) 
    ...
    if(! out_side){
              ...
              frappe.call({
                      method: "prime.prime.doctype.obs.obs.refer", //dotted path to server method
                      args: {
          ...
          let d = new frappe.ui.Dialog({
              ...
              primary_action(values) {
                    let diagnose = d.get_value("diagnose")
                      ...
                      frappe.call({
                              method: "prime.prime.doctype.obs.obs.transfer", //dotted path to server method
                              args: {
                                  "diagnose" : diagnose,
                                  'patient':frm.doc.patient,
                                  'name': frm.doc.name,
