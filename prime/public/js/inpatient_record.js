        ...
        if(frm.doc.status == "Discharge Scheduled"){
            var refurn_btn=  frm.add_custom_button(__("Discharge"), function(){
                ...
                frappe.msgprint({
					...
					frappe.call({
						method: "prime.api.inpatient_record.check_out_inpatient", //dotted path to server method
						args: {
                    ...
                    // frappe.call({
                    //     method: 'prime.api.inpatient_record.check_out_inpatient',
					// 	args: {
					// 		inpatient_record: frm.doc.name,
		...
		if (!frm.doc.__islocal) {
			if (frm.doc.status == 'Admitted' && !frm.doc.accepted_or_rejected) {
			 ...
			 () => {
				 // action to perform if Yes is selected
							 frappe.call({
								method: "prime.api.inpatient_record.inpatient_record",
								args: {
            ...
            if(frm.doc.status == "Admission Scheduled"){
			 ...
			 () => {
				 // action to perform if Yes is selected
							 frappe.call({
								method: "prime.api.ot_prepation.make_ot_schedule",
								args: {
									docname: frm.doc.name,
