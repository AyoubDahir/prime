...
def add_service_items(so, doc):
    ...
    for child_table in ("lab_test_prescription", "procedure_prescription" , "radiology_prescription", "services_prescription"):
        for row in doc.get(child_table):
            ...
            so_item.qty = 1
            if child_table=="radiology_prescription":
                pass
                # so_item.description = row.type
...
def get_item_and_is_billable(row):
    ...
    elif row.doctype == "Check Up prescription":
        return frappe.get_cached_value(
            "Package Template", row.package_template, ("item", "is_billable")
        )
