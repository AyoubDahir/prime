...
frappe.pages['ordered-tests'].on_page_load = function(wrapper) {
	new OrderedTests(wrapper)
}

OrderedTests = Class.extend(
	{
		init:function(wrapper){
			this.page = frappe.ui.make_app_page({
				parent : wrapper,
				title: "New Ordered Tests",
				single_column : true
			});
			this.groupbyD = []
			...
			let myf = this
			this.colms = []
			frappe.realtime.on('sample_msg', (data) => {
				// alert("in realtime")
				myf.setupdata_table()
					})
		...
		let tbldata = []
		frappe.call({
        method: "prime.api.labtsamples.get_ordered", //dotted path to server method
		...
		tbldata.forEach(row => {
			...
			row['status'] = "<span style='color:red'>Not Collected</span>"
			// row['action'] = btnhml
			
			new_data.push(row)
		})
		// console.log(columns)
this.table = new Tabulator("#dordered_tests", {
			// layout:"fitDataFill",
...
<div id="dordered_tests" style = "min-width : 100%"></div>
