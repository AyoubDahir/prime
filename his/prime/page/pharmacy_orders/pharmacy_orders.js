frappe.pages['pharmacy-orders'].on_page_load = function(wrapper) {	
...
OpdOrder = Class.extend(
	{
		...
		let tbldata = []
		frappe.call({
        method: "prime.api.get_orders.get_pharmacy_orders", //dotted path to server method
		...
		//  let fields = frappe.get_meta("Sales Order").fields
			...
			// {title:"Doctor Name", field:"ref_practitioner" ,  headerFilter:"input",},
			{title:"Total", field:"total" , formatter:"money",  headerFilter:"input",formatterParams:{
    decimal:",",
    thousand:".",
    symbol:"$",
    symbolAfter:"p",
    ...
    precision:false,
			...
			{title:"Depart", field: "source_order"},
			{title:"User", field: "full_name", headerFilter:"input",},
		...
		tbldata.forEach(row => {
			...
			`
				// console.log(row.allow_credit)
				// if(row.allow_credit ) {
					btnhml += `	<button class='btn btn-danger ml-2' onclick = "credit_sales('${row.name}')"> Credit</button>`
					
				// }// Open
				// else{
				// 	btnhml += `	<button class='btn btn-danger ml-2' onclick = "frappe.set_route('customer' , '${row.customer}')"> Link To Debtor</button>`
		...
		// console.log(columns)
this.table = new Tabulator("#pharorder", {
			// layout:"fitDataFill",
 ...
 make_sales_invoice : function(source_name) {
	...
	frappe.model.open_mapped_doc({
		method: "prime.api.make_invoice.make_sales_invoice",
 ...
 make_credit_invoice : function(source_name) {
	frappe.model.open_mapped_doc({
		method: "prime.api.make_invoice.make_credit_invoice",
		source_name: source_name
	})
}
	}
...
<div id="pharorder" style = "min-width : 100%"></div>
