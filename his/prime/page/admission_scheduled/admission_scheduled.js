...
function cancel_admision(inpatient_record, patient){
    ...
    () => {
		...
		frappe.call({
			...
			method: 'prime.api.admission_schd.cancel_admision',
			args:{
...
function admit_p(inpatient_record, bed,patient_name,practitioner, type){
	frappe.call({
		...
		method: 'prime.api.admit.admit_p',
		args:{
		...
		primary_action(values) {
			...
			//   let patient = d.get_value("patient")
			  ...
			  var args = {
				...
				frappe.call({
						method: "prime.api.admission_schd.schedule_inpatient", //dotted path to server method
						args: {
							args: args
						},
						callback: function(r) {
