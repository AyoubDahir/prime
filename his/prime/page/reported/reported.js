...
frappe.pages['reported'].on_page_load = function(wrapper) {
	new Reported(wrapper)
}

Reported = Class.extend(
	{
		init:function(wrapper){
			this.page = frappe.ui.make_app_page({
				parent : wrapper,
				title: "Reporting Samples",
				single_column : true
			});
			this.groupbyD = []
		...
		let tbldata = []
		frappe.call({
        method: "prime.api.labtsamples.get_lab_result", //dotted path to server method
		...
		// console.log(columns)
this.table = new Tabulator("#lab_reporting", {
			// layout:"fitDataFill",
...
<div id="lab_reporting" style = "min-width : 100%"></div>
