...
EmergencyNurse = Class.extend(
	{
		...
		let tbldata = []
		frappe.call({
        method: "prime.api.get_orders.get_que_em", //dotted path to server method
		...
		//  let fields = frappe.get_meta("Sales Order").fields
			...
			{title:"Doctor", field:"practitioner" ,  headerFilter:"input"},
...
this.table = new Tabulator("#opdorder", {
		 ...
		 this.table.on("rowClick", function(e ,rows){
			...
			//  row.toggle_actions_menu_button(row.table.getSelectedData().length > 0);
			 ...
			 frappe.new_doc("Patient History" , {source_order: "E.R" , patient: rows._row.data.patient ,   practitioner: rows._row.data.practitioner})
