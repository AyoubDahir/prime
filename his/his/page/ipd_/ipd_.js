...
IPD = Class.extend(
	{
		...
		frappe.db.get_list('Inpatient Record', {
			fields: ['patient','patient_name', 'room' , 'bed' , 'admitted_datetime' , 'admission_practitioner' , 'diagnose'],
			filters: {
				status: 'Admitted'
			},
			limit : 1000
...
this.table = new Tabulator("#admited", {
		...
		this.table.on("rowClick", function(e ,rows){
		   ...
		   //  row.toggle_actions_menu_button(row.table.getSelectedData().length > 0);
			...
			frappe.new_doc("Patient History" , {patient: rows._row.data.patient, source_order: "IPD", practitioner: rows._row.data.admission_practitioner})
