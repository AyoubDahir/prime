...
frappe.ui.form.on('Discharge And Clearance', {
			...
			if(frm.doc.patient_balance){
				...
				var refurn_btn=  frm.add_custom_button(__("Receipt"), function(){
					let d = new frappe.ui.Dialog({
						...
						primary_action(values) {
							...
							// 			frappe.call({
							// 				method: "prime.api.inpatient_record.clear_patient", //dotted path to server method
							// 				args: {
							// 					inpatient_record : frm.doc.inpatient_record,
				...
				d.show()
					...
					let d = new frappe.ui.Dialog({
					    ...
					    primary_action(values) {
								frappe.call({
					            method: "prime.api.inpatient_record.clearance", //dotted path to server method
					            args: {
	...
	customer: function(frm){
		...
		frappe.call({
			method: "prime.api.api.patient_clearance",
			args: {
		...
		// frappe.call({
        //     method: "prime.api.api.patient_clearance", //dotted path to server method
        //     args: {
