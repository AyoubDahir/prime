...
frappe.ui.form.on('General Surgery', {
	...
	if(! out_side){
            ...
            frappe.call({
                    method: "prime.prime.doctype.general_surgery.general_surgery.refer", //dotted path to server method
                    args: {
                ...
                let d = new frappe.ui.Dialog({
                    ...
                    primary_action(values) {
                          let diagnose = d.get_value("diagnose")
                            ...
                            frappe.call({
                                    method: 'prime.prime.doctype.general_surgery.general_surgery.transfer', //dotted path to server method
                                    args: {
                                        "diagnose" : diagnose,
                                        'patient':frm.doc.patient,
                                        'name': frm.doc.name,
