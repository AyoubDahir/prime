...
frappe.ui.form.on('Clinical Procedure', {
	...
	refresh: function(frm) {
		...
		// 			frappe.call({
		// 				method: 'prime.prime.doctype.aneasthesia_sheet.aneasthesia_sheet.transfer',
		// 				args: {
...
frappe.ui.form.on('Lab Test Template', {
	refresh(frm) {
		...
		frm.set_query('item', 'inventory', function() {
            return {
                // query: "prime.api.dp_drug_pr_link_query.my_custom_query",
                filters: {
                    is_stock_item: 1
                }
...
// 	frappe.call({
// 		method: "prime.api.vitals.get_vital", //dotted path to server method
// 		args :{ "patient": patient
// 			},
// 		callback: function(r) {
...
function transfer_balance(from_c,amount){
	...
	let d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
			...
			frappe.call({
                method: "prime.api.journal_entry.transfer_balance",
                args: {
...
function statement(party){
    let d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "prime.api.api.get_report_content",
                args: {
...
function receipt(party , outstanding , group){
	...
	// 				function(){
	// 					frappe.call({
	// 						method: "prime.api.create_p_e.payment_re",
	// 						args: {
...
function statementsuppier(party){
    let d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "prime.api.api.get_report_content_2",
                args: {
...
function statementemployee(party){
    let d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "prime.api.api.get_report_content_3",
                args: {
	...
	new frappe.ui.Dialog({
		...
		primary_action: function(values) {
			...
			frappe.call({
				...
				method: "prime.api.create_inv.make_ipd_sales_invoice",
				freeze: true,
				// args: {
...
function renew(name){
	alert("Renew")
	frappe.call({
		method: "prime.api.Que_to_make_sales_invove.renew",
		args: {
